...
/**
 * Edits a meal's foods and grams; every number of the meal is recalculated. `corrections` lists the
 * edits the app offers to remember (personal food memory).
 */
export const PUT = handle<{ id: string }>(async (request, { id }) => {
  ...
  const { meal: saved, corrections } = await replaceItems(meal, body);
  return Response.json({ meal: await toMealWithItems(saved), corrections });
});
